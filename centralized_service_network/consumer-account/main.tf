...
# --- centralized_service_network/consumer-account/main.tf ---

# ---------- SECRETS MANAGER - OBTAINING LATTICE SERVICE NETWORK ----------
data "aws_secretsmanager_secret" "service_network" {
  arn = "arn:aws:secretsmanager:${var.aws_region}:${var.central_aws_account}:secret:service_network"
}

data "aws_secretsmanager_secret_version" "service_network" {
  secret_id = data.aws_secretsmanager_secret.service_network.id
}

locals {
  service_network = nonsensitive(jsondecode(data.aws_secretsmanager_secret_version.service_network.secret_string))
}

# ---------- AWS RAM - ACCEPTING VPC LATTICE SERVICE NETWORK ----------
resource "aws_ram_resource_share_accepter" "share_accepter" {
  share_arn = local.service_network.ram_share
}

# ---------- AMAZON VPC LATTICE VPC ASSOCIATIONS ----------
# Option 1: Using VPC Lattice TF module
module "vpc_lattice_vpc_association" {
  source  = "aws-ia/amazon-vpc-lattice-module/aws"
  version = "0.0.2"

  service_network = { identifier = local.service_network.id }
...
# Option 2: Directly using VPC TF module
module "vpc2" {
  source  = "aws-ia/vpc/aws"
  version = "4.3.0"
  ...
  vpc_lattice = {
    service_network_identifier = local.service_network.id
